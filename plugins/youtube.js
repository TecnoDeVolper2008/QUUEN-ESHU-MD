...
Function({
  ...
  const writer = await addAudioMetaData(await toAudio(await fs.readFileSync(media.file), 'mp4'), media.thumb, media.title, `Quueneshu-md`, 'Quueneshu Official');
  ...
  const writer = await addAudioMetaData(await toAudio(await fs.readFileSync('./' + response.file), 'mp4'), response.thumb, response.title, `Quueneshu-md`, 'Quueneshu Official');
  return await send(message, writer, ytId[1]);
  }
...
Function({
  ...
  if (isUrl(match) && match.includes('youtu')) {
    ...
    try {
      ...
      const writer = await addAudioMetaData(await toAudio(await fs.readFileSync(media.file)), thumb, media.title, `${config.BOT_INFO.split(";")[0]}`, 'Quueneshu Official');
      return await send(message, writer, ytId[1]);
    } catch {
      const response = await getJson('https://api.adithyan.xyz/ytaudio?id=' + ytId[1]);
      if (!response.status) return await message.send('*Failed to download*');
...
Function({
    ...
    if (isUrl(match) && match.includes('youtu')) {
        ...
        if (fileSizeInMB > 10) {
            ...
            const thumb = await getBuffer(await getYoutubeThumbnail(ytId[1]));
            const audioBuffer = await getBuffer(result.url);
            const writer = await addAudioMetaData(await toAudio(audioBuffer), thumb, result.title, `${config.BOT_INFO.split(";")[0]}`, 'Quueneshu Official');
        ...
        if (fileSizeInMB > 10) {
            ...
            const audioBuffer = await getBuffer(result.url);
            const file = await addAudioMetaData(audioBuffer, thumb, result.title, `${config.BOT_INFO.split(";")[0]}`, 'Quueneshu Official');
